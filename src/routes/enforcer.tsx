...
function EnforcerPage() {
  ...
  if (matchRoute({ to: "/enforcer/citations", fuzzy: false })) {
    return <Outlet />;
  }
  const officer = user;
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <Field label="Evidence photo" hint="Simulated">
          <button
            onClick={() =>
              setPhotoName(
                photoName
            ...
            className={cn(
              "flex h-28 w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed text-sm font-medium active:scale-[0.99]",
              photoName
                ? "border-paid-foreground/40 bg-paid text-paid-foreground"
                : "border-input bg-card text-muted-foreground",
            )}
          >
            <Camera className="size-6" />
            {photoName
              ? `Attached · ${photoName} (tap to remove)`
